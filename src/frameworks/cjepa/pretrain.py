"""Pre-training code for C-JEPA framework proposed by Mo & Tong in 2024. 
Reference:
- Shentong Mo and Shengbang Tong (2024). Connecting Joint-Embedding Predictive Architecture with Contrastive Self-supervised Learning. NeurIPS2024.
- https://arxiv.org/abs/2410.19560
Key Ideas: 
- Incorporating VIC-Reg regularization into the JEPA framework to enhance representation learning.
- Removing the stop-gradient operation on target encoder without model collapse.
"""
...
from src.frameworks.cjepa.loss import cjepa_loss

def main(params, args):
    """Pre-training function for C-JEPA.
    ...
    - v_loss: variance loss in VIC-Reg
    - c_loss: covariance loss in VIC-Reg
    - i_loss: invariance loss in VIC-Reg
    """
    ...
    if params['framework'].get('mlp_projector', None) is not None:
        mlp_projector = init_projector(
            in_dim=target_encoder.embed_dim,
            **params['framework']['mlp_projector']
        ).to(device)
        logger.info(f"MLP Projector Info: {mlp_projector}")
    ...
    optimizer = get_pretrain_optimizer(
        ...
        mlp_projector=mlp_projector if params['framework'].get('mlp_projector', None) is not None else None,
    )
    ...
    # -- get loss weights & momentum usage
    loss_betas = params['framework'].get('loss_betas', {})
    beta_sim = loss_betas.get('sim', 25)
    beta_std = loss_betas.get('std', 25)
    beta_cov = loss_betas.get('cov', 1)
    beta_vicreg = loss_betas.get('vicreg', 0.001)
    use_momentum = params['framework'].get('use_momentum', True)
    logger.info(f"Loss Weights: beta_sim: {beta_sim}, beta_std: {beta_std}, beta_cov: {beta_cov}, beta_vicreg: {beta_vicreg}")
    logger.info(f"Using momentum for target encoder: {use_momentum}")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        time_vicreg_meter = AverageMeter()
        ...
        for step, (batch, masks_enc, masks_pred) in enumerate(train_loader):
            ...
            B = len(imgs)
            Me, Mp = masks_enc[0].shape[0], masks_pred[0].shape[1]
            Me = len(masks_enc)
            Mpred = len(masks_pred)
            # -- mask meter step
            ...
            with torch.amp.autocast("cuda", enabled=opt_params['use_bfloat16']):
                ...
                start.record()
                context_z = encoder(imgs, masks_enc)  # (B*Nenc, d)
                target_z_pred = predictor(context_z, masks_enc, masks_pred)  # (B*Npred*Nenc, Mp, d)
                end.record()
                torch.cuda.synchronize()
                time_enc_meter.step(start.elapsed_time(end)/1000.0)  # seconds
                
                # -- 3. compute ijepa loss
                jepa_loss = F.smooth_l1_loss(target_z_pred, target_z, reduction='none').mean(dim=(1, 2))
                loss = jepa_loss.mean()
                
                # -- 4. compute vicreg loss
                start.record()
                vicreg_loss = cjepa_loss(
                    target_z.view(B, Mpred, Mp, target_dim).contiguous(),  # (B, Npred, Mp, D)
                    target_z_pred.view(B, Mpred, Mp, target_dim).contiguous(),  # (B, Npred, Mp, D)
                ...
                loss += beta_vicreg * vicreg_loss
                end.record()
                torch.cuda.synchronize()
                time_vicreg_meter.step(start.elapsed_time(end)/1000.0)  # seconds
            ...
            vicreg_loss_meter.step(vicreg_loss)
            loss_meter.step(loss)

            # -- log info
            log_step = epoch * ipe + step
            if step % params['logging']['log_step_freq'] == 0:
                logger.info(f"Epoch [{epoch}/{num_epochs}] Step [{step}/{ipe}] Total Loss: {loss_meter.val:.4f} ({loss_meter.avg:.4f}) "
                            f"JEPA loss: {jepa_loss.mean().item():.4f} VIC-Reg Loss: {vicreg_loss_meter.val:.4f} ({vicreg_loss_meter.avg:.4f}) "
                            ...
                            f"Time_vicreg: {time_vicreg_meter.avg:.4f}s ({time_vicreg_meter.avg:.4f}s)")
                ...
                if use_csv:
                    log_dict = {
                        ...
                        'vicreg_loss': vicreg_loss_meter.val,
                        'jepa_loss': jepa_loss.mean().item(),
                        ...
                        'time_vicreg': time_vicreg_meter.avg,
                        'time_vicreg_avg': time_vicreg_meter.avg,
                ...
                if use_tensorboard:
                    tb_log_dict = {
                        ...
                        'JEPA/loss': jepa_loss.mean().item(),
                        'VICReg/loss': vicreg_loss_meter.val,
                        ...
                        'Time/time_vicreg_step': time_vicreg_meter.avg,
                        'Time/time_vicreg_avg': time_vicreg_meter.avg,
                ...
                if use_wandb:
                    wandb_log_dict = {
                        'Loss/step': loss_meter.val,
                        'Loss/avg': loss_meter.avg,
                        'Loss/jepa_loss': jepa_loss.mean().item(),
                        'Loss/vicreg_loss': vicreg_loss_meter.val,
                        'Mask/mask_enc': mask_enc_meter.avg,
                        'Mask/mask_pred': mask_pred_meter.avg,
                        'Time/time_enc_step': time_enc_meter.avg,
                        'Time/time_enc_avg': time_enc_meter.avg,
